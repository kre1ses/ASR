...
class ConvModule(nn.Module):
    ...
    def forward(self, x: torch.Tnesor) -> torch.Tensor:
...
class Conv2dSubsampling(nn.Module):
    ...
    def forward(self, x: torch.Tensor, input_lengths: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        x = self.conv(x)  # (batch_size, out_dim, seq_len', embed_dim')
        batch_size, channels, subsampled_seq_len, subsampled_embed_dim = x.size()
        x = x.transpose(1, 2).contiguous().view(batch_size, subsampled_seq_len, channels * subsampled_embed_dim)
        x = self.linear(x)
        x = self.dropout(x)
