...
class RelativeSinusoidalPositionEmbedding(nn.Module):
    ...
    def apply_rpe(self, tens: torch.Tensor) -> torch.Tensor:
        if self.pe is not None:
            if self.pe.size(1) >= tens.size(1) * 2 - 1:
                return
        ...
        self.pe = pe.to(device=tens.device, dtype=tens.dtype)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        self.apply_rpe(x)
        
        encodings = self.pe[:, self.pe.size(1)//2 - x.size(1) + 1 : self.pe.size(1)//2 + x.size(1), :]
        return encodings